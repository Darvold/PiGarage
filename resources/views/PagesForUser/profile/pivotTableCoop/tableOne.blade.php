<table>
   <thead>
    <tr>
        <th colspan="10" style="font-size: 28px; text-align: center; background-color: white;">Сводная таблица электричества</th>
    </tr>
    <tr>
        <th style="text-align: center;"  rowspan="2">Месяц</th>
        <th colspan="2" >Показания счётчика</th>
        <th rowspan="2">Потреблено кВт гаражами</th>
        <th rowspan="2">Установка потерь</th>
        <th rowspan="2">Потреблено кВт с потерями</th>
        <th rowspan="2">Тариф</th>
        <th rowspan="2">К оплате за электричество</th>
        <th rowspan="2">Внесено</th>
        <th rowspan="2">Долг</th>
    </tr>
    <tr>
        <td style="background-color: hsla(0, 0%, 80%, 0.6); border: 1px solid black;">Начало периода</td>
        <td style="background-color: white;"></td>
    </tr>
</thead>
<tbody>
    <tr>
        <td>Январь</td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>

    </tr>
    <tr>
        <td>Февраль</td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>

    </tr>
    <tr>
        <td>Март</td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>

    </tr>
    <tr>
        <td>Апрель</td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>

    </tr>
    <tr>
        <td>Май</td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>

    </tr>
    <tr>
        <td>Июнь</td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>

    </tr>
    <tr>
        <td>Июль</td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>

    </tr>
    <tr>
        <td>Август</td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>

    </tr>
    <tr>
        <td>Сентябрь</td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>

    </tr>
    <tr>
        <td>Октябрь</td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>

    </tr>
    <tr>
        <td>Ноябрь</td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>

    </tr>
    <tr>
        <td>Декабрь</td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>

    </tr>
</tbody>
<tfoot>
    <tr>
        <td style="font-size: 28px;">Итого:</td>
        <td style="border: 1px solid black;"></td>
        <td style="border: 1px solid black; background-color: hsla(0, 0%, 80%, 0.6); font-size: 27px;">Конец периода</td>
        <td style="border: 1px solid black;"></td>
        <td></td>
        <td style="border: 1px solid black;"></td>
        <td style="border: 1px solid black;"></td>
        <td style="border: 1px solid black;"></td>
        <td style="border: 1px solid black;"></td>
        <td style="border: 1px solid black;"></td>
    </tr>
</tfoot>
<!-- Добавьте остальные месяцы по аналогии -->
</table>
