    ...
    <script>
        $(document).ready(function () {
            ...
            function sendAjaxRequestBlockKw(currentYearInput) {
                $.ajax({
                    url: '{{ route('ChairmanMyCoopRate.index', ['idCoop' => $idCoop]) }}',
                    type: "GET",
                    data: {
                    ...
                    success: function (response) {
                        ...
                        for (var i = 0; i < months.length; i++) {
                            ...
                            var html = `<form method="post" action="{{ route('ChairmanMyCoopRatePost.store', ['idCoop' => $idCoop]) }}" class="form_month" id="${formId}">
                        @csrf
                            <input type="hidden" class="id_year" name="id_year" value="${year}">
                            ...
                            if (groupedByMonth[monthNumber]) {
                                ...
                                <div>
                                <input type="tel" pattern="[0-9]*[.]?[0-9]+" name="tariff_value" id="tariff_value"
                                maxlength="2" oninput="this.value=this.value.replace(/\D/g,'')" value="${blockMessage.tariff_value}">
                                <span>%</span>
                                <button type="submit" class="monthButton">Сохранить</button>
                                </div>`;
                                });
