@extends('layouts.profileChairman', ['ProfileCoopBlocksStyles' => ['pageBlocks.css', 'scroll_blocks.css']])
...
<div class="main_block_coop">
    <div class="main_head">
        ...
        <a href="{{route('ChairmanMyCoopBlocks.index', ['idCoop' => $idCoop])}}" class="active">Гаражные блоки</a>
    </div>
    <div class="main_body">
        <div class="left_block">
            <span class="span_text">Кооператив: {{$coopData->name}}</span> <br>
            <span class="span_text">Мои гаражные блоки:</span>
            <div class="flex_container">
                <div class="main_garages_block">
                    ...
                    @endforeach
                    <form method="post"
                    action="{{route('ChairmanMyCoopNewBlocks.store', ['idCoop' => $idCoop])}}">
                    @csrf
                    ...
                    <button class="button_new_block" type="submit">
                        <span class="new_span_number_block">Добавить новый блок</span>
                    </button>
                </form>
            </div>
        </div>
    ...
    <div class="right_block">
        <form method="post" action="{{route('ChairmanMyCoopNewBlocks.store', ['idCoop' => $idCoop])}}" class="default_form_kw">
            ...
            <input type="tel" pattern="[1-9][0-9]?" name="default_kw" maxlength="2" oninput="this.value=this.value.replace(/\D/g,'')" value="">
            <span>%</span>
        ...
        <div class="table_kw_change">
            <div class="head_block_table">
                <div class="button_form_ajax">
                    ...
                    <div class="block_2">
                        <span class="number_block_output">Гаражный блок: №</span>
                    </div>
                </div>
            </div>
...
<script>
    $(document).ready(function () {
        ...
        function sendAjaxRequestBlockKw(currentYearInput, blockNumber) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $('.number_block_output').text('Гаражный блок: №' + response.number_block);
