...
<div class="main_block">
    ...
    <div class="main_body">
      ...
      <div class="main_right_block">
          <div class="head_main_right_block">
              @if($garageData)
                  @foreach($garageData as $gData)
                    <button class="garage-btn" data-garage="{{$counterGarageBtn}}">Гараж №{{$counterGarageBtn}}<span class="delete_garage"><img src="{{asset('icons/user/cross.svg')}}"></span></button>
                      @php
                          $counterGarageBtn++;
                      @endphp
                  @endforeach
          ...
          @else
                  <button class="garage-btn" data-garage="1">Гараж №1<span class="delete_garage"><img src="{{asset('icons/user/cross.svg')}}"></span></button>
          @endif
              ...
              @foreach($garageData as $gData)
                  <div class="block_input active" id="block_input_{{ $counter }}">
                      <div class="right_block_input">
                          ...
                          <input type="tel" pattern="[0-9]{1,10}" title="Только цифры" name="number_meter" required maxlength="20" oninput="this.value=this.value.replace(/\D/g,'')" value="{{ $gData['number_meter'] }}">
                      </div>
                      <div class="right_block_input">
                          <label>Номер гаража:</label>
                          <input type="tel" pattern="[0-9]{1,10}" title="Только цифры" name="number_garage" required maxlength="20" oninput="this.value=this.value.replace(/\D/g,'')" value="{{ $gData['number_garage'] }}">
                      </div>
                      <div class="right_block_input">
                          <label>Номер блока:</label>
                          <input type="tel" pattern="[0-9]{1,10}" title="Только цифры" name="number_block" required maxlength="20" oninput="this.value=this.value.replace(/\D/g,'')" value="{{ $gData['number_block'] }}">
                      </div>
                  </div>
                  @php
                      $counter++;
          ...
          @else
              <div class="block_input active" id="block_input_1">
                  <div class="right_block_input">
                      <label>Номер счётчика: </label>
                      <input type="tel" pattern="[0-9]{1,10}" title="Только цифры" name="number_meter" required maxlength="20" oninput="this.value=this.value.replace(/\D/g,'')" value="">
                  </div>
                  <div class="right_block_input">
                      <label>Номер гаража:</label>
                      <input type="tel" pattern="[0-9]{1,10}" title="Только цифры" name="number_garage" required maxlength="20" oninput="this.value=this.value.replace(/\D/g,'')" value="">
                  </div>
                  <div class="right_block_input">
                      <label>Номер блока:</label>
                      <input type="tel" pattern="[0-9]{1,10}" title="Только цифры" name="number_block" required maxlength="20" oninput="this.value=this.value.replace(/\D/g,'')" value="">
                  </div>
              </div>
              <div class="error_msg">
        ...
        <div class="search_coop_main">
            <div class="head_search_coop">
                <div class="head_search_coop_input">
                    <input type="text" name="name_coop" placeholder="Название кооператива" required>
                    <button>Найти</button>
                </div>
                <div class="flex_region_and_city_select">
                    <div class="select_wrp">
                        <select class="js-select-region" name="region" placeholder="Выберите регион" style="min-width: 230px;">
                            <option value=""></option>
                        </select>
                    </div>
                    <div class="select_wrp">
                        <select class="js-select-city" name="city" placeholder="Выберите город" style="min-width: 230px;">
                            <option value=""></option>
                        </select>
                    </div>
                </div>
                        ...
                        success: function (response) {
                            ...
                            if (Array.isArray(blockMessages) && blockMessages.length > 0) {
                                for (var i = 0; i < blockMessages.length; i++) {
                                    console.log(blockMessages[i].name);
