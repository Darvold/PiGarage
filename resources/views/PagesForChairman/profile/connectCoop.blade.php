@extends('layouts.mainChairman', ['ProfileConnectCoopStyles' => ['connectCoop.css', 'map_marker.css', 'select2.min.css']])
...
@section('profile')
    ...
    <div class="main_block">
        <div class="main_head">
            <a href="{{route('ChairmanConnectCoop.index')}}" class="active">Поиск кооперативов</a>
            <a href="{{route('MyApplicationToCoop.index')}}" class="">Мои заявки</a>
        </div>
        <div class="main_body">
            <div class="center_widthe_container">
                <div class="left_block">
                    <div class="information_text">
                        <span
                            style="font-size: 27px; font-weight: bold; ">Присоединение к кооперативу - быстро и легко!</span>
                ...
                <div class="main_right_block">
                    ...
                    @if($garageData)
                        @foreach($garageData as $gData)
                            <div class="block_input active" data-garage="{{$counterGarageBtn}}" id="block_input_{{ $counter }}">
                            ...
                            @endphp
                        @endforeach
                        <div class="not_garage">
                            <div class="checkbox-wrapper-13">
                                <label for="c1-13">Без гаража:</label>
                    ...
                    @else
                    <div class="block_input active" data-garage="1" id="block_input_1">
                        <div class="right_block_input">
                            <label>Номер счётчика: </label>
                            <input type="tel" pattern="[0-9]{1,10}" title="Только цифры" name="number_meter"
                            required maxlength="20" oninput="this.value=this.value.replace(/\D/g,'')"
                    ...
                    <div class="error_msg" style="border: none;"></div>
                    @endif
                </div>
            </div>
                ...
                <div class="search_coop_main">
                    <div class="head_search_coop">
                        <div class="head_search_coop_input">
                            <input type="text" id="fname" name="name_coop" pattern=".{3,}"
                                   placeholder="Название кооператива" required>
                            <button>Найти</button>
                        </div>
                    ...
                    function ajaxSelectCoop() {
                        if (selectedRegion && selectedCity) {
                            ...
                            <button data-id-point="${blockMessages[i].latitude},${blockMessages[i].longitude}">Показать на карте</button>
                                        ...
                                        $('.block_name_result_coop .span_text').remove();
                                        $('#load-more').hide();
                                        $('.block_name_result_coop').append(`<span class="span_text">Ошибка, похоже нет кооперативов с заданными параметрами. Введённое название: ${nameCoopSelect}</span>`);
                                    }
                                },
            ...
            <script type="text/javascript">
                $(document).ready(function () {
                    ...
                    $(document).on('click', '#addGarageBtn', function (event) {
                        ...
                        const newBlock = `
                        <div class="block_input active" data-garage="${garageCount}" id="block_input_${garageCount}">
                        <div class="right_block_input">
                        <label>Номер счётчика: </label>
                        ...
                        $('.not_garage').insertBefore('.error_msg');
                        ...
                        $(`.garage-btn[data-garage="${garageCount}"]`).css({
                            'color': 'white',
                            'background-color': 'rgb(19, 105, 192)',
                            'border': '3px solid rgb(19, 105, 192)'
                        });
