    ...
    <script>
        $(document).ready(function () {
            ...
            function sendAjaxRequestUser() {
                $.ajax({
                    url: '{{ route('ApplicationsMessageReject.index', ['id' => Auth::id()]) }}',
                    type: "GET",
                    data: {
                        idCoop: idCoopValue,
                        _token: '{{ csrf_token() }}',
                    ...
                    success: function (response) {
                        ...
                        if (coopMessages !== null && coopMessages.length > 0) {
                            coopMessages.forEach(function (coopMessage) {
                                ...
                                var html = `<div class="block_applications" id="formMessage_${coopMessage.user_id}">
                                <div class="img_center_left">
            ...
            function sendAjaxRequestParticipant() {
                $.ajax({
                    url: '{{ route('ApplicationsMessageGarageReject.index', ['id' => Auth::id()]) }}',
                    type: "GET",
                    data: {
                        idCoop: idCoopValue,
                        _token: '{{ csrf_token() }}',
